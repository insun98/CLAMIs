...
public class DataFeasibilityChecker {
	...
	public void computeNumberOfGroups(Instances instancesByCLA, Instances instances, double percentileCutoff, String positiveLabel) {
		...
		int[] samePrediction = new int[instances.numInstances()];

		int numOfGroup = 0;
		int valueOfMaxVote = 0;
		int numOfMaxVote = 0;
		int numberOfSamePrediction = 0;
		double alpha = 1.224;
		...
		for (int instIdx1 = 0; instIdx1 < instances.numInstances(); instIdx1++) {

			if (samePrediction[instIdx1] == numOfGroup || samePrediction[instIdx1] == 0) {
				numberOfSamePrediction++;
			}
